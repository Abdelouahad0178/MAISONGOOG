document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderTable(articlesToRender) {
        ...
        articlesToRender.forEach(article => {
            ...
            if (isAlternativeView) {
                ...
                // Ensure srcImg key is at the end
                const keys = Object.keys(article);
                const values = Object.values(article);
                const srcImgIndex = keys.indexOf("srcImg");
                keys.push(keys.splice(srcImgIndex, 1)[0]);
                values.push(values.splice(srcImgIndex, 1)[0]);
                
                row.innerHTML = `
                    <td colspan="6">
                        <table class="nested-table">
                            ${keys.map((key, index) => `
                                <tr><th>${key}</th><td style="${key === 'promotion' ? 'color:red;' : key === 'stock' ? 'color:green;' : ''}">${key === 'srcImg' ? `<img src="${values[index]}" alt="${article.article}" data-ref="${article.ref}" class="article-img">` : values[index]}</td></tr>
                            `).join('')}
                        </table>
                    </td>
                `;
    ...
    toggleDataBtn.addEventListener('click', function () {
        ...
        if (isDataView) {
            articlesTableHead.innerHTML = `
                <th colspan="6">Données des Articles</th>
            `;
        } else {
            articlesTableHead.innerHTML = `
                <th class="small-column">Réf</th>
